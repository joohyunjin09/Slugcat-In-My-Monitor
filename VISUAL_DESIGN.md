...
| 아이콘/표식 | Bone grey | #E8EBE8 | 단색 픽셀 아이콘과 작은 현재 명령 표식 |

## 3. Object Rendering Specifications

- 명령 휠의 도넛 면과 아이콘은 절반 해상도의 투명 캔버스에 렌더한 뒤 2배 최근접 확대한다.
- 세 섹터는 외곽선 없이 투명한 각도 간격으로 분리한다. 중앙은 완전히 비워 슬러그캣의 얼굴과 행동을 가리지 않는다.
- 세 명령은 글자 없이 구분한다. `멈춰`는 두 개의 정지 기둥, `움직여`는 오른쪽을 보는 재생 삼각형, `날 따라와`는 오른쪽을 가리키는 화살표로 표현하며 모두 도넛과 같은 2×2 픽셀 격자를 사용한다.
- 도넛 차트와 명령 판정 영역은 초기 배치에서 시계 방향 60°로 이동해 아래쪽 갈라진 틈의 중심선이 화면 아래 방향인 90°를 정확히 가리킨다. 아이콘은 각 섹터와 함께 위치만 이동하며 도형 방향은 회전하지 않는다.
- 아이콘은 런타임 코드에서 절차적으로 그리며 별도 비트맵이나 글꼴 자산을 추가하지 않는다.
...
- 2-second recognition check: 2초 안에 슬러그캣, 서로 다른 세 아이콘, 현재 호버 면의 순서가 읽혀야 한다

### 7.2 Limits on Familiar Template Symbols

- Adopted familiar elements (max 2): 방사형 메뉴, 재생 삼각형과 방향 화살표 기호군
- Replaced unique element: 일반 앱 아이콘과 카드 테두리를 Rain World식 생물 흔적 픽셀 문양과 작은 상태 표식으로 대체
