...
**Rain World의 슬러그캣이 Windows 바탕화면을 돌아다니는 데스크톱 펫입니다.**
슬러그캣은 모니터와 실제 창의 경계를 바닥이나 벽처럼 이용하고, 걷기·점프·낙하·벽
오르기·휴식 같은 행동을 스스로 선택합니다. 마우스로 잡아 옮기거나 던질 수 있으며,
최대 8마리를 동시에 실행할 수 있습니다.

![여러 슬러그캣이 바탕화면을 돌아다니는 모습](docs/media/readme/example-preview.gif)

> [!IMPORTANT]
> **Rain World를 구매하여 PC에 설치한 사용자만 정상적으로 사용할 수 있습니다.**
> 이 프로그램은 슬러그캣 기본 외형과 스킨을 자체적으로 제공하지 않고, 사용자의 로컬
...
Rain World, 스팀 또는 Unity를 동시에 실행할 필요는 없습니다. 게임 실행 파일이나 DLL을
...
- **여러 슬러그캣:** 생성·선택·삭제와 캐릭터별 이동 능력 및 특수 능력
...
- **원본 캐릭터 사운드:** 설치된 Rain World 사운드 뱅크에서 발걸음·점프·엎드리기·구르기·몸 충돌음과 구현된 다운푸어 특수 능력음 재생. 바닥·벽·배 슬라이딩 사운드는 재생하지 않습니다.
- **안정적인 동시 재생:** 고정된 48kHz WinMM 장치 하나에서 최대 128개의 가벼운 재생 커서를 소프트웨어 믹싱하며 충돌·능력·울음용 32개 슬롯을 별도로 남깁니다. 재사용하는 40ms 출력 링과 피크 리미터가 장치 반복 생성, 버퍼 언더런, 겹침 클리핑을 방지합니다. 여러 레이어로 구성된 `PLAYALL` 사운드는 모든 레이어를 재생할 수 있을 때만 함께 시작합니다. 일회성 사운드는 자연스럽게 끝까지 재생하고, 행동 루프는 종료 순서를 보존한 짧은 클릭 방지 처리 뒤 정지합니다.
- **Push To Meow 연동:** 설치된 모드의 슬러그캣·슬러그펍별 울음, 눈감기 얼굴과 위쪽 얼굴 이동을 재현하며 창술가는 시선을 유지한 채 꼬리를 두 번 튕깁니다.
- **음량 제어:** 설정 창의 0–200% 마스터 음량 슬라이더와 설정 창·트레이 메뉴의 즉시 음소거. 첫 실행은 음소거이며 이후에는 마지막 음소거 선택을 복원합니다.

사운드 인덱싱, FSB5 PCM/Vorbis 읽기와 디코딩, 재생 음성 정리는 전용 작업 스레드에서 수행합니다.
시뮬레이션은 길이가 제한된 명령 큐에 요청만 넣으며, 해제된 클립은 모든 Slugcat이
공유하는 24 MiB LRU 캐시 안에서 관리합니다.

Push To Meow가 설치되어 있으면 저장소에 음원을 복사하지 않고 모드의 `sounds.txt`,
`custom_meows.json`을 읽고 현재 지원 Slugcat/Slugpup 울음 WAV만 백그라운드에서 준비합니다.
펫은 처음 12~30초 뒤부터 포만도에
따라 약 24~85초 간격으로 울며, 수면·기절·드래그 중에는 울지 않습니다.
...
| 슬러그캣 | 실행 이름 | 현재 구현된 특징 |
|---|---|---|
| Survivor | `white` | 표준 이동 |
| Monk | `yellow` | 완만한 이동 특성 |
| Hunter | `red` | 높은 신체 능력치 |
| Gourmand | `gourmand` | 구르기 |
| Artificer | `artificer` | 폭발 도약, 충격파와 자폭 이펙트 |
| SpearMaster | `spearmaster` | 바늘 창 생성 및 투척 |
| Rivulet | `rivulet` | 빠른 달리기· 높은 점프 |
| Saint | `saint` | 혀를 이용한 이동 |
...
- **PC에 설치된 정품 Rain World**
  ...
  - 프로그램이 원본 슬러그캣 atlas를 읽을 수 있도록 게임 파일이 온전해야 합니다.
...
- 스팀 창작마당에서 스킨을 받을 경우 **스팀 클라이언트**

스팀은 창작마당 설치 위치를 자동으로 찾을 때만 필요합니다. Rain World와 모드를
직접 설치하고 경로를 지정한 경우, 이 프로그램을 실행하는 동안 스팀을 켜둘 필요는
...
시스템 트레이의 슬러그캣 아이콘을 왼쪽 클릭하면 설정 패널이 열립니다.
...
- **슬러그캣을 마우스로 집거나 놓기**
- **트레이 아이콘을 왼쪽 클릭하여 설정 탭 띄워 설정하기**
- **먹이를 고르고 원하는 위치에서 마우스 왼쪽 버튼을 눌러 먹이 놓기**

시스템 단축키와 충돌하지 않도록 전역 단축키는 등록하지 않습니다. 먹이와 슬러그캣
오버레이는 일반 데스크톱 클릭을 가로채지 않으며, 슬러그캣을 직접 잡을 때만 왼쪽 드래그
...
![실험적 슬러그캣 스킨 패널](docs/media/readme/skinPanel-rework.png)

스킨 편집기에서는 머리, 얼굴, 몸, 팔, 엉덩이, 다리, 꼬리와 표식 파츠를
...
프로그램은 Rain World의 `mods`, `mergedmods` 및 발견된 스팀 창작마당 폴더에서
...
- **기본 외형이 깨지거나 절차형 외형만 표시됨**: 스팀 등에서 Rain World 게임 파일
  무결성을 검사한 뒤 다시 실행하세요.
- **DMS 스킨이 보이지 않음**: DMS 설치 여부, 스킨 모드의 Remix 활성화 상태와
  PNG/TXT atlas 쌍을 확인하고 창작마당 모드를 새로 고침하세요.
...
[MIT License](LICENSE)는 이 프로젝트의 기여자가 독립적으로 작성하고 필요한 이용허락
권한을 보유한 코드와 기타 자료에만 적용됩니다. Rain World의 소스 코드나 오브젝트 코드,
캐릭터, 이미지, 음원, 게임 자산, 명칭, 상표 및 그 밖의 제3자 자료에는 적용되지
않습니다. 이 저장소는 그러한 자료에 관한 어떠한 이용허락도 부여하지 않습니다. 제3자
구성요소에는 각각의 라이선스와 이용조건이 계속 적용됩니다.
